...
`v1.4.0`이 출시되었습니다. 프리셋 목록, 상세 화면, 수동 복사, 선택적 root 적용, 선택적 플로팅 도우미가 구현되어 있으며, UI는 **20개 언어**로 현지화되고 앱 내에서 표시 언어를 전환할 수 있습니다(도구 모음의 번역 아이콘으로 언어 메뉴를 엽니다). MIT 라이선스의 FOSS 지향 앱이며 GMS 의존성이 없습니다.
