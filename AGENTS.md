...
> Status: **dev env ready (`plan_implement_steps.md`); v1 decisions locked; coding plan in `plan_coding_steps.md`. M-A (scaffold) + M-B (presets, 12 carriers/16 presets) + M-C (persistence) + M-D (UI) + M-E (apply strategies: manual + libsu root, behind `ApplyStrategy` seam) + M-E.1 (root-apply hardening: verified writes, opt-in `su` probe, dedup re-apply — `plan_review_M-E.md` P1/P2) + M-F (i18n: en + ja `strings.xml`) + M-G (test harness + CI: strict `just ci` gate, GitHub Actions on arm64 macOS, `README.md`) + M-H (polish & release prep: versioning 1.0.0, `CONTRIBUTING.md`, en+ja fastlane store metadata; closed all `plan_review_M-E.md` P4 items) done & tested — 56 JVM + 7 instrumented green; root telephony write verified via `adb root`; `assembleDebug` builds a v1.0.0 APK (2026-06-27). 🎉 v1 code-complete; remaining = store screenshots + F-Droid recipe (non-code).**
